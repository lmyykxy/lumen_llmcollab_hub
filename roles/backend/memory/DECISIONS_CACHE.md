...
```text
Q1：后端模型层需要支持按需 skill 读取,降低每轮上下文常驻量。
Q2：SkillRegistry / read_skill 只能用白名单 skill_id,不能任意文件读取。
Q3：context budget 需要新增 skills_loaded_count / skill_ids / skills_total。
Q4：skill 内容、prompt、routing reason 不得返回前端。
Q5：优先做 P7.1 规则版 skill routing,不强制每轮增加 LLM planner 调用。
```

## 2026-04-28 P7 深度设计

```text
Q1：后端 P7.1 优先实现规则路由 + State Resolver,不引入高延迟 planner。
Q2：SkillRegistry 必须白名单化,read_skill 不能任意读文件路径。
