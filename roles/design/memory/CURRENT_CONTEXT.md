...
```text
P6 ImageIntentBuilder 已授权。用户补充拍板：小七本人相关画像必须参考服务器固定图片 `/root/companion/backend/res/xiaoqi.png`。

设计侧长期规则：
1. 小七本人画像、头像、自拍、照片、她现在的样子等,不得只靠文字描述随机生成。
2. 眼睛仍是固定身份特征,眼镜仍是可变配饰。
3. 若生图工具暂不支持 reference image,必须在交付中说明限制,不得声称已实现参考图一致性。
```
