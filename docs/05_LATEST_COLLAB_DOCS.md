...
> 五个模型开始任何任务前，先读本文件。
> 当前结构版本：v1.2
> 本版变更：补充 P6.3/P7/P8 复审结论；P7 接受用户拍板 LLM 主导架构但需补验收，P8 仅在 P7 验收通过后条件开工 v1。
...
| DOC-MODEL-RPT-P62 | 模型层 P6.2 POV 撤回修正报告 | `docs/collaboration/model-layer/20_模型_P6.2修正报告.md` | Model | done(已重审) | v0.1 | 用户审视实际生成图后拍板 B:删 _POV_SCENE_OVERRIDES,"画你窗外/画你的房间" 走 xiaoqi self → 用 xiaoqi.png + 场景在 prompt;PM 已在 `27_*.md` 重审:含“画你/你的X”默认小七相关并用 xiaoqi.png,纯“画窗外/画房间一角”默认纯场景;290 单测;实测中等关系下生成图为"小七在窗边",身份锚点全保留 |
...
| DOC-MODEL-RPT-P7 | 模型层 P7 LLM 主导上下文编排交付报告 | `docs/collaboration/model-layer/25_模型_P7交付报告.md` | Model | done(待用户验收补充) | v0.1 | 用户拍板 LLM 主导版:6 skill 文件(prompts/skills/)+ SkillRegistry + read_skill tool(enum/dedup/max=3/fail closed)+ ContextOrchestrator 注入 593 chars Skill Index + core_card 精简 3899→3383(world.md / relationships.md 详细搬到 skill)+ output_filters 加 11 关键词防工具透明化 + 318 单测全过 + 3 核心 smoke(闲聊不调 / 房间问 / 关系试探);PM 已接受 LLM 主导方向,但需补 image/state/max=3/latency/overuse 验收 |
| DOC-MODEL-PROP-P8 | 模型层 P8 记忆 Skill 系统提案 | `docs/collaboration/model-layer/26_模型_P8记忆Skill系统提案.md` | Model | conditional next | v0.1 | 用户跟模型商讨导出:三层记忆架构(P5 fact 检索 + 私有 memory skill 文件 + 共同 memory DB per-user)+ SkillRegistry 自动扫描可扩展 + meta-skill `xiaoqi.memory.index` 分层索引 + v1 只读路径(写路径 v2 每晚 LLM 批处理);PM 接受方向,但必须等 P7 验收通过后再开工 P8 v1 |
| DOC-PM-MODEL-P7P8 | P6.3/P7/P8 复审与 P8 条件授权 | `docs/collaboration/model-layer/27_PM_P6.3_P7_P8复审与P8条件授权.md` | PM | active | v0.1 | PM 复审:P6.3 方向接受但需 prompt 截断 audit + subject 规则收紧;P7 接受用户拍板 LLM 主导架构但当前不最终验收,需补 image/state/max=3/latency/overuse;P8 方向接受,仅在 P7 验收通过后条件开工 v1,且 v1 只做读路径 |
...
| DOC-PM-MODEL-P7-DEEP | P7 深度设计：角色上下文编排与按需 Skill 读取 | `docs/collaboration/model-layer/22_PM_P7深度设计_角色上下文编排方案.md` | PM | active | v0.2 | 对比现有 Skills/RAG/Router/Plugin/Tool 模式后定案 Hybrid Purposeful Context Orchestrator；补充具体实现蓝图：建议文件路径、SkillRegistry/ContextOrchestrator 接口、路由伪代码、预算常量、日志字段、测试文件 |
...
| 模型层 / 小七人格切换 | `docs/collaboration/model-layer/` | active | PM 已接受 P7 用户拍板的 LLM 主导 read_skill 架构,但 P7 需先补 image/state/max=3/latency/overuse 验收;用户将与模型验收 P7;P8 三层记忆架构方向接受,但必须等 P7 验收通过后再做 P8 v1,且 v1 只做读路径 |
...
docs/collaboration/model-layer/20_模型_P6.2修正报告.md                   # P6.2 用户拍板修正:删 POV override 让"画你窗外/画你的房间"走 xiaoqi self;冲突已在 27_PM 复审中解决;实测 smoke 含小七的窗边画面
...
docs/collaboration/model-layer/25_模型_P7交付报告.md                     # P7 交付:6 skill 文件 + SkillRegistry + read_skill tool + ContextOrchestrator + 3 类核心 smoke 通过(闲聊/房间/关系)+ 318 单测全过;PM 接受 LLM 主导方向,待补验收
docs/collaboration/model-layer/26_模型_P8记忆Skill系统提案.md             # P8 用户跟模型商讨导出提案:三层记忆架构(P5 fact + 私有 memory skill 文件 + 共同 memory DB per-user)+ SkillRegistry 自动扫描 + meta-skill 分层索引 + v1 只读路径(v2 每晚 LLM 批处理写)+ 6 项用户决策(Q1-Q6);等 P7 验收后开工
docs/collaboration/model-layer/27_PM_P6.3_P7_P8复审与P8条件授权.md        # PM 复审:P7 接受 LLM 主导但需补验收;P8 仅在 P7 验收通过后条件开工 v1
```
...
19. P7 上下文构造应更有目的性：常驻只保留身份、语气、安全、最小状态和必要历史；房间、生图、日记、关系细节等改为按 intent/state 读取 skill。`read_skill` 只能读白名单 skill_id,不得任意读文件或向前端暴露 skill 内容。
20. P7 深度设计定案：不做纯全量 prompt、纯向量 RAG、纯 LLM planner 或多 agent handoff；先做 P7.1 规则版 SkillRegistry + ContextOrchestrator,首批只做 bedroom/window/image_self/image_scene/diary/relationship_voice 6 个 skill,每轮最多 3 个 skill。
21. 用户与模型后续拍板改为 P7 LLM 主导版：LLM 看 Skill Index 后自行决定是否 read_skill(0-3 个)。PM 接受该架构方向,替代第 20 条中的规则优先执行路线,但 P7 当前还不能最终验收。
22. P7 验收必须补 image/state/max=3/latency/overuse/工具透明化 smoke：`画一下你窗外现在的样子` 必须使用 xiaoqi.png；`画窗外的雨` 不使用 xiaoqi.png；`你今天在做什么` 需结合 state 读对应 skill。
23. P8 三层记忆架构方向接受,但仅在 P7 经用户验收通过后条件开工 P8 v1。P8 v1 只做 read path、private memory seed、shared_memories 表和 per-user 隔离测试；不做夜间 LLM 写入路径,不宣称完整共同记忆系统完成。
```

### PM 文档质量硬约束

后续 PM 给模型/后端/前端的规划文档不得只有概念，必须写到可执行级别。
...
5. 明确核心算法、流程或伪代码。
6. 明确失败策略和边界条件。
7. 明确测试清单、smoke 输入输出和验收标准。
8. 明确风险与下一步。
```

如果某项暂时无法确定，文档必须写明“待模型/后端评估的问题”，不能用抽象概念替代实现方案。
