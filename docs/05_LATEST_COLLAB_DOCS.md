...
> 五个模型开始任何任务前，先读本文件。
> 当前结构版本：v1.1
> 本版变更：补充 PM 文档质量硬约束；所有给模型/后端/前端的规划文档必须写到可执行级别，包含文件路径、数据结构、算法/伪代码、测试和验收标准，避免只有概念。
...
| DOC-MODEL-PROP-P8 | 模型层 P8 记忆 Skill 系统提案 | `docs/collaboration/model-layer/26_模型_P8记忆Skill系统提案.md` | Model | proposal(等 PM 复审) | v0.1 | 用户跟模型商讨导出:三层记忆架构(P5 fact 检索 + 私有 memory skill 文件 + 共同 memory DB per-user)+ SkillRegistry 自动扫描可扩展 + meta-skill `xiaoqi.memory.index` 分层索引 + v1 只读路径(写路径 v2 每晚 LLM 批处理) + 6 项用户决策(Q1 索引写概述 / Q2 自动扫描 / Q3 模型先写种子 / Q4 三层架构 / Q5 分层索引 / Q6 v2 写) + ~6.5 session 工作量;PM 接受后立即开工 v1 |
...
| 模型层 / 小七人格切换 | `docs/collaboration/model-layer/` | active | P3-P6.3 + P7 已交付;**P8 用户跟模型商讨导出提案**:三层记忆架构(P5 fact 检索 + 私有 memory skill 文件 + 共同 memory skill DB per-user)+ SkillRegistry 改自动扫描 + meta-skill 分层索引 + v1 只读路径(写路径 v2 每晚 LLM 批处理);跟 P7 兼容,等 PM 复审 |
...
docs/collaboration/model-layer/26_模型_P8记忆Skill系统提案.md             # P8 用户跟模型商讨导出提案:三层记忆架构(P5 fact + 私有 memory skill 文件 + 共同 memory DB per-user)+ SkillRegistry 自动扫描 + meta-skill 分层索引 + v1 只读路径(v2 每晚 LLM 批处理写)+ 6 项用户决策(Q1-Q6)
```
